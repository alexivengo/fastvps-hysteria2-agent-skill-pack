...
Both deploy wrappers stream the same shared remote bash resource to the VPS. If deployment behavior needs to change, patch `scripts/remote_deploy_fastvps_hysteria2.sh` first and only touch the wrappers when argument handling or local artifact generation changes.
...
- `scripts/deploy_fastvps_hysteria2.sh`: deterministic FastVPS deployer with client artifact generation.
- `scripts/deploy_fastvps_hysteria2.ps1`: native PowerShell deployer for Windows operators.
